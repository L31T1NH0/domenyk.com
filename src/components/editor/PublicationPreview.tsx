  ...
  useEffect(() => {
    ...
    const timer = setTimeout(async () => {
      try {
        ...
        setPreview(`<!doctype html><html class="${attribute(theme)}"><head><meta name="viewport" content="width=device-width,initial-scale=1"><meta http-equiv="Content-Security-Policy" content="default-src 'none'; frame-src https://www.youtube-nocookie.com; img-src https: http:; style-src 'unsafe-inline' ${location.origin}; font-src ${location.origin} data:">${links}<style>html,body{margin:0;padding:0;min-height:0;background:transparent}body{padding:16px}</style></head><body><div data-public-shell><div class="post-content">${result.html}</div></div></body></html>`)
        setError("")
      } catch (error) {
