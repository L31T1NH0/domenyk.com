...
export function AdminNotesTable({ notes }: { notes: AdminNote[] }) {
  ...
  return <section className="admin-list">
    ...
    <header className="admin-list-header admin-note-row"><span>Nota</span><span>SEO</span><span>Views reais</span><span>Publicada</span><span aria-hidden /></header>
    {filtered.map((note) => <Link href={`/admin/notes/${note._id}`} key={note._id} className="admin-list-row admin-note-row">
      ...
      <span className="admin-list-number">{note.metrics.directViews}</span>
